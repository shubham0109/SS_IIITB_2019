...
int main(int argc, char const *argv[])
{
    ...
    printf("CPU time used to execute getpid(): %llu nsec\n", ns); 
    double usec = (double) ns / (double)1000;
    printf("CPU time used to execute getpid(): %lf usec or %e usec\n", usec, usec);
    double sec = (double)ns / (double)1000000000;
	printf("CPU time used to execute getpid(): %lf sec or %e sec.\n", sec, sec);
	exit(EXIT_SUCCESS);
}